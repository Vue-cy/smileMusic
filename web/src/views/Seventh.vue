  ...
  <div>
    ...
    <div class="list">
      <van-tabs v-model="active" sticky style="margin-top: 30px;margin-left: 25px">
        ...
        <van-tab>
          ...
          <ul class="comment">
            <li v-for="(data,index) in man" :key="index" style="padding: 20px;border-bottom: 1px solid darkgrey">
              ...
              <p style="color: darkgrey">{{ data.timeStr }} {{ data.time }}</p>
              <p><i class="iconfont icon-dianzan"></i> {{ data.likedCount }}
        ...
        <van-tab title="收藏者">收藏着 3</van-tab>
      </van-tabs>
    </div>
    <div class="space"></div>
  </div>
...
Vue.filter('timefilter', (date) => {
  return moment(date).format('YYYY-MM-DDD')
})
...
export default {
  ...
  computed: {
    ...mapState(['isbtnShow', 'playList', 'list', 'index', 'loveId', 'master'])
  },
  methods: {
    getIndex (index) {
      if (index < 9) {
    ...
    ...mapMutations(['updatebtnShow', 'deleteCurrentTime', 'updateList', 'deletePlayList', 'updateLoveId'])
  },
  mounted () {
    this.$nextTick(() => {
      console.log('1111')
      service({
        url: `/user/playlist?uid=${this.master.userId}`
      }).then(res => {
        // console.log(res.data)
        this.updateLoveId(res.data.playlist[0].id)
      })
      axios.get(`http://localhost:3000/playlist/detail?id=${this.loveId}`)
        .then(res => {
          console.log(res.data)
      ...
      axios.get(`http://localhost:3000/comment/playlist?id=${this.loveId}`)
        .then(res => {
